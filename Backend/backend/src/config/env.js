...
const loadEnv = () => {
  ...
  if (result.error) {
    console.warn(
      `⚠️  Could not load ${envFile}, falling back to system environment variables.`,
    );
  } else {
...
const validateEnv = () => {
  ...
  const required = useConnectionUrl
    ? []
...
const logConnectionInfo = () => {
  ...
  console.log(
    `LLM_PROVIDER: ${process.env.LLM_PROVIDER || "⚠️  NOT SET (will default to mock)"}`,
  );
