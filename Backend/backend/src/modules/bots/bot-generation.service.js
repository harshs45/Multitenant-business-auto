...
const generateBotTx = async (userId, payload) => {
  ...
  return await sequelize
    .transaction(async (t) => {
      // ─── 1. Create Business ───────────────────────────────
      logger.info('Step 1/9: Creating business entry');
      const businessType =
        BUSINESS_TYPE_MAP[payload.businessType] ||
      ...
      logger.info('Step 2/9: Initializing default subscription');
      ...
      // ─── 3. Create Bot ────────────────────────────────────
      logger.info('Step 3/9: Creating bot record and generating API key');
      const tone =
        TONE_MAP[payload.toneOfVoice] ||
        payload.toneOfVoice?.toLowerCase() ||
        'professional';

      const responseLanguage =
        LANGUAGE_MAP[payload.responseLanguage] ||
      ...
      const bot = await Bot.create(
        {
          ...
          welcomeMessage:
            payload.welcomeMessage || 'Hi there! How can I help you today?',
          tone,
          responseLanguage,
      ...
      // ─── 8. Load FAQs + Build Prompt + Publish ────────────
      logger.info('Step 8/9: Loading FAQs, building system prompt, and publishing bot');

      const faqs = await KnowledgeBaseFAQ.findAll({
        where: { botId: bot.id },
        transaction: t,
      });

      bot.systemPrompt = buildSystemPrompt(
        bot,
        business,
        audienceConfig,
        features,
      ...
      const apiBase =
        process.env.API_BASE_URL || 'https://botforge-api-m6d4.onrender.com';
