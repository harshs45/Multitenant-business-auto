    ...
    async function init() {
      ...
      try {
        ...
        if (convRes.success) {
          ...
          setTotalConversations(convRes.data.total);
          setTotalPages(convRes.pagination.totalPages);
        }
      } catch (err: any) {
  ...
  if (loading && currentPage === 1) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-muted/20">
        ...
        <p className="text-muted-foreground font-medium">Loading conversation history...</p>
      </div>
    );
  }
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-1">
            <h1 className="text-3xl font-bold tracking-tight text-foreground flex items-center gap-3">
              <MessageSquare className="text-blue-500" size={32} /> Interaction History
            </h1>
              ...
              {conversations.map((conv, i) => (
                ...
                >
                      ...
                      <div className="space-y-1">
                         <h4 className="font-bold text-foreground flex items-center gap-2">
                           ...
                           <span className={cn(
                             ...
                             conv.status === 'active' ? "bg-emerald-500/10 text-emerald-500" : "bg-muted text-muted-foreground"
                           )}>
                             <Circle size={8} fill="currentColor" /> {conv.status}
                           </span>
                         </h4>
                         <div className="flex items-center gap-4 text-xs font-medium text-muted-foreground">
                            <span className="flex items-center gap-1"><Clock size={12} /> {new Date(conv.createdAt).toLocaleDateString()}</span>
                            {conv.lead && (
                               <span className="flex items-center gap-1 text-emerald-500">
                                 <UserIcon size={12} /> {conv.lead.name || conv.lead.email}
                               </span>
                            )}
                         </div>
                      </div>
                   ...
                   <div className="flex items-center gap-4">
                      <button className="text-xs font-bold text-primary opacity-0 group-hover:opacity-100 transition-all flex items-center gap-1">
